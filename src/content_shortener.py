...
class ContentShortener:
    ...
    def shorten_content(self, content: str, target_length: int) -> str:
        ...
        if len(content) <= target_length:
            self.logger.debug(f"Content fits target length: {len(content)} <= {target_length}")
            return content
        ...
        end_target = target_length - beginning_target - middle_target  # Remaining 30% for end
        
        # Sample from beginning
        beginning_chunks = self._get_chunks_up_to_length(chunks[:len(chunks)//3], beginning_target)
        
        # Sample from middle
        middle_start = len(chunks) // 3
        middle_end = 2 * len(chunks) // 3
        middle_chunks = self._get_chunks_up_to_length(chunks[middle_start:middle_end], middle_target)
        
        # Sample from end
        end_chunks = self._get_chunks_up_to_length(chunks[2*len(chunks)//3:], end_target)
        ...
        self.logger.info(f"Content shortened: {len(content)} → {len(shortened)} chars ({len(chunks)} chunks → {len(beginning_chunks + middle_chunks + end_chunks)} chunks)")
        self.logger.debug(f"Section lengths - Beginning: {len(beginning_chunks)}, Middle: {len(middle_chunks)}, End: {len(end_chunks)}")
        
        return shortened
    
    def _get_chunks_up_to_length(self, chunks: List[str], target_length: int) -> List[str]:
        """
        ...
        for chunk in chunks:
            ...
            if current_length >= target_length * 0.9:  # Stop at 90% to leave room for separators
                break
