...
class PdfHandler(BaseHandler):
    ...
    def extract_content(self, file_path: Path) -> ExtractedContent:
        ...
        try:
            ...
            # Fallback to pypdf
            self.logger.warning(f"MarkItDown failed for {file_path}: {markitdown_error}, falling back to pypdf")
            try:
                text_content = []
                ...
                self.logger.error(f"Both MarkItDown and pypdf failed for {file_path}: {pypdf_error}")
                raise pypdf_error
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                if reader.metadata:
                    ...
                    self.logger.debug(f"Extracted PDF metadata from {file_path}: {metadata}")
        ...
        return ExtractedContent(
            content=content,
            metadata=metadata,
            extraction_method=f"{extraction_method}_with_pdf_metadata" if metadata else extraction_method,
