...
from src.handlers import (
    TxtHandler, MarkdownHandler, DocxHandler, 
    PdfHandler, HtmlHandler, JsonHandler
)
...
class TestHtmlHandler:
    ...
    def test_extract_content_with_metadata(self, mock_convert):
        ...
        assert result.metadata['notes'] == 'This is a test HTML document for testing HTML handler'
    ...
    def test_extract_content_encoding_fallback(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False, encoding='latin-1') as f:
        ...
        try:
            with patch('builtins.open', side_effect=[
                ...
                with patch('src.handlers.html_handler.convert_to_markdown', return_value="# Test"):
                    result = handler.extract_content(temp_path)
...
class TestJsonHandler:
    ...
    def test_extract_content_success(self):
        ...
        assert result.metadata['notes'] == 'This is a test document created for unit testing the JSON handler functionality.'
        assert 'test' in result.metadata['tags']
    ...
    def test_extract_content_missing_original_text(self):
        ...
        try:
            with pytest.raises(ValueError, match="missing required 'original_text' field"):
...
class TestDocxHandler:
    ...
    def test_extract_content_basic(self, mock_markitdown):
        ...
        assert result.extraction_method in ["markitdown_with_docx_properties", "markitdown_only", "markitdown_fallback"]
...
class TestPdfHandler:
    ...
    @patch('src.handlers.pdf_handler.MarkItDown')
    @patch('pypdf.PdfReader')
    def test_extract_content_markitdown_fallback_to_pypdf(self, mock_pdf_reader, mock_markitdown):
        """Test PDF extraction fallback from MarkItDown to pypdf."""
        handler = PdfHandler()
        test_file = Path('tests/fixtures/sample.pdf')
        
        # Mock MarkItDown to fail
        mock_markitdown.return_value.convert.side_effect = Exception("MarkItDown failed")
...
class TestHandlerIntegration:
    ...
    def test_no_extension_conflicts(self):
        ...
        assert len(all_extensions) == len(set(all_extensions)), "Handlers have conflicting extensions"