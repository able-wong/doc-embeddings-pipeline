...
class TestSparseEmbeddingProviderFactory:
    ...
    def test_create_unknown_provider(self):
        ...
        with pytest.raises(ValueError, match="Unknown sparse embedding provider: unknown_provider"):
            create_sparse_embedding_provider(config)
...
class TestSpladeProviderConfiguration:
    ...
    def test_splade_config_missing(self):
        ...
        with patch('src.sparse_embedding_providers.SpladeProvider._initialize_model') as mock_init:
...
class TestSpladeProviderInterface:
    ...
    def test_generate_sparse_embedding_interface(self):
        ...
        config = SparseEmbeddingConfig(
            provider="splade",
            splade=SpladeConfig(model="test-model", device="cpu")
        )
        ...
        with patch('src.sparse_embedding_providers.SpladeProvider._initialize_model'):
            ...
            with patch.object(provider, 'generate_sparse_embedding', return_value={"indices": [1, 2], "values": [0.5, 0.3]}):
    ...
    def test_get_info_interface(self):
        ...
        with patch('src.sparse_embedding_providers.SpladeProvider._initialize_model'):
            ...
            # Mock get_info
            with patch.object(provider, 'get_info', return_value={"provider": "splade", "status": "ready"}):
