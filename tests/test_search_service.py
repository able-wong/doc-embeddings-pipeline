...
class TestSearchServiceCore:
    ...
    def test_get_capabilities_sparse_store_no_provider(
        ...
        search_service = SearchService(
            mock_vector_store, mock_embedding_provider, None  # No sparse provider
        )
    ...
    def test_search_multi_strategy_all_strategies_no_sparse(
        ...
        assert results["semantic"] == [{"id": "doc1", "score": 0.8, "payload": {"chunk_text": "test result"}}]
...
class TestSearchServicePrivateMethods:
    ...
    def mock_vector_store(self):
        ...
        mock_store.search_dense.return_value = [{"id": "dense", "score": 0.8, "payload": {}}]
        mock_store.search_sparse.return_value = [{"id": "sparse", "score": 0.9, "payload": {}}]
        mock_store.search_hybrid_with_text.return_value = [{"id": "hybrid", "score": 0.85, "payload": {}}]
        return mock_store
    ...
    def mock_sparse_provider(self):
        mock_provider = Mock()
        mock_provider.generate_sparse_embedding.return_value = {"indices": [1, 2], "values": [0.5, 0.3]}
        return mock_provider
...
class TestSearchServiceErrorScenarios:
    ...
    def test_exact_search_not_implemented_error(
    ...
    def test_hybrid_search_not_implemented_fallback(
        self, mock_embedding_provider
    ):
        """Test hybrid search falls back when NotImplementedError raised."""
        mock_vector_store = Mock()
        mock_vector_store.supports_sparse_vectors.return_value = True
        mock_vector_store.search_hybrid_with_text.side_effect = NotImplementedError("Not supported")
        mock_vector_store.search_dense.return_value = [{"id": "fallback", "score": 0.7, "payload": {}}]
    ...
    def test_hybrid_search_real_error_propagation(
        ...
        mock_vector_store.search_hybrid_with_text.side_effect = RuntimeError("Database connection failed")
