...
def convert_single_file(
    json_path: Path, output_dir: Path, overwrite: bool = False
) -> Tuple[bool, str]:
    """
    ...
    if html_path.exists() and not overwrite:
        return (
            False,
            f"HTML file already exists: {html_path.name} (use --overwrite to replace)",
        )
...
Examples:
  ...
  python convert-json-docs-to-html.py . ./html
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing HTML files"
    )
